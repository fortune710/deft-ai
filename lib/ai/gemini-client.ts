...
function buildOnboardingPrompt(data: OnboardingFormData): string {
  const goalLabel =
...
function parseAIResponse(text: string): AIContext {
  try {
...
export async function generateAIContextClient(data: OnboardingFormData): Promise<AIContext> {
  const apiKey = process.env.NEXT_PUBLIC_GEMINI_API_KEY;
  ...
  if (!apiKey) {
    throw new Error('Gemini API key not configured. Please restart the dev server.');
  }
  ...
  try {
    ...
    throw new Error(error?.message || 'Failed to generate AI content strategy');
  }
}
